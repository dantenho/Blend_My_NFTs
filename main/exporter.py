...
from .helpers import Loader, TextColors
...
def save_completed(full_single_dna, a, x, batch_json_save_path, batch_to_generate):
    ...
    file_name = os.path.join(batch_json_save_path, f"Batch{batch_to_generate}.json")
    batch = json.load(open(file_name))
...
def render_and_save_nfts(input):
    ...
    if input.fail_state:
        ...
        nfts_in_batch, hierarchy, batch_dna_list = get_batch_data(
            input.failed_batch, input.batch_json_save_path
        )
    ...
    # If user is generating the normal way:
        ...
        nfts_in_batch, hierarchy, batch_dna_list = get_batch_data(
            input.batch_to_generate, input.batch_json_save_path
        )
    ...
    for a in batch_dna_list:
        ...
        def match_material_dna_to_material(single_dna, material_dna, materials_file):
            ...
            for attribute, variant, material in zip(
                list_attributes, list_dna_deconstructed, list_material_dna_deconstructed
            ):
                ...
                if material != "0":  # If material is not empty
                    for variant_m in materials_file:
                        if variant == variant_m:
                            ...
                            materials_list = list(
                                materials_file[variant_m]["Material List"].keys()
                            )

                            material = materials_list[
                                int(material) - 1
                            ]  # Subtract 1 because '0' means empty mat
                            break
        ...
        if input.enable_materials:
            material_dna_dictionary = match_material_dna_to_material(
                single_dna, material_dna, materials_file
            )
            ...
            for var_mat in list(material_dna_dictionary.keys()):
                if material_dna_dictionary[var_mat] != "0":
                    if not materials_file[var_mat]["Variant Objects"]:
                        ...
                        """
                        metadata_material_dict[var_mat] = material_dna_dictionary[
                            var_mat
                        ]

                        for obj in bpy.data.collections[var_mat].all_objects:
                            selected_object = bpy.data.objects.get(obj.name)
                            selected_object.active_material = bpy.data.materials[
                                material_dna_dictionary[var_mat]
                            ]
        ...
        batch_folder = os.path.join(
            input.nft_batch_save_path, "Batch" + str(input.batch_to_generate)
        )
            ...
            def render_animation():
                ...
                if not input.enable_debug:
                    if input.animation_file_format == "MP4":
                        ...
                        input.animation_file_format == "PNG"
                        or input.animation_file_format == "TIFF"
                    ):
        ...
        with open(
            os.path.join(bmnft_data_folder, "Data_" + name + ".json"), "w"
        ) as outfile:
        ...
        save_completed(
            full_single_dna, a, x, input.batch_json_save_path, input.batch_to_generate
        )
    ...
    batch_info = {
        "Batch Render Time": batch_complete_time,
        "Number of NFTs generated in Batch": x - 1,
