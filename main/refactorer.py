...
def reformat_nft_collection(refactor_panel_input):
    complete_coll_path = os.path.join(
        refactor_panel_input.save_path, "Blend_My_NFTs Output", "Complete_Collection"
    )
    ...
    for folder in batch_list:
        batch_info = json.load(
            open(
                os.path.join(
                    refactor_panel_input.nft_batch_save_path, folder, "batch_info.json"
                )
            )
        )
        collection_info[os.path.basename(folder)] = batch_info
        collection_info["Total Time"] = (
            collection_info["Total Time"] + batch_info["Batch Render Time"]
        )

        file_list_dirty = os.listdir(
            os.path.join(refactor_panel_input.nft_batch_save_path, folder)
        )
        filelist = remove_file_by_extension(file_list_dirty)

        for mediaTypeFolder in filelist:
            if mediaTypeFolder != "batch_info.json":
                media_type_folder_dir = os.path.join(
                    refactor_panel_input.nft_batch_save_path, folder, mediaTypeFolder
                )
    ...
    log.info(
        f"All NFT files stored and sorted to the Complete_Collection folder in {refactor_panel_input.save_path}"
    )
