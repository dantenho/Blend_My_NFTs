...
def create_cardano_metadata(
    ...
    for i in nft_variants:
        meta_data_dict_cardano["721"]["<policy_id>"][name][i] = strip_nums(
            nft_variants[i]
        )
    ...
    send_metadata_to_json(meta_data_dict_cardano, cardano_metadata_path, name)
...
def createSolanaMetaData(
    ...
    for i in nft_variants:
        dictionary = {"trait_type": i, "value": strip_nums(nft_variants[i])}
        attributes.append(dictionary)

    # Material Variants and Attributes:
    for i in material_attributes:
        dictionary = {"trait_type": i, "value": material_attributes[i]}
        attributes.append(dictionary)

    # Custom Fields:
    if enable_custom_fields:
        for i in custom_fields:
            dictionary = {"trait_type": i, "value": custom_fields[i]}
            attributes.append(dictionary)

    metadata_dict_solana["attributes"] = attributes
    metadata_dict_solana["collection"] = {"name": "", "family": ""}
    ...
    send_metadata_to_json(metadata_dict_solana, solana_metadata_path, name)
...
def create_erc721_meta_data(
    ...
    send_metadata_to_json(metadata_dict_erc721, erc721_metadata_path, name)
