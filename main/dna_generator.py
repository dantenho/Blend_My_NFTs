...
from . import helpers, logic, material_generator
...
def generate_nft_dna(
    ...
    def single_complete_dna():
        ...
        if enable_logic:
            single_dna = logic.logicafy_dna_single(
                hierarchy, single_dna, logic_file, enable_rarity
            )
            log.debug(f"\n================\nLogic DNA: {single_dna}")

        if enable_materials:
            single_dna = material_generator.apply_materials(
                hierarchy, single_dna, materials_file, enable_rarity
            )
            log.debug(
    ...
    def create_dna_list():
        ...
        for i in range(collection_size):
            ...
            dna_set_return |= {
                "".join([dna_push_to_list()])
                for _ in range(collection_size - len(dna_set_return))
            }
        ...
        for i in dna_list_non_formatted:
            dna_list_formatted.append(
                {i: {"complete": False, "order_num": dna_counter}}
            )
...
def make_batches(collection_size, nfts_per_batch, save_path, batch_json_save_path):
    """
    ...
    if batch_list:
        for i in batch_list:
            ...
            if os.path.exists(batch):
                os.remove(os.path.join(batch_json_save_path, i))
    ...
    for i in range(num_batches):
        ...
        with open(
            os.path.join(batch_json_save_path, f"Batch{i + 1}.json"), "w"
        ) as outfile:
...
def send_to_record(
    ...
    def create_nft_data():
        try:
            ...
            if enable_rarity:
                helpers.check_rarity(
                    data_dictionary["hierarchy"],
                    data_dictionary["dna_list"],
