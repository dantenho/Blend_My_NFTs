...
def apply_rule_to_dna(
    hierarchy, deconstructed_dna, if_dict, result_dict, result_dict_type, enable_rarity
):
    """
    ...
    if result_dict_type == "NOT":
        for a in result_dict:  # For each Attribute in the NOT rule dictionary
            ...
            full_att = list(result_dict[attribute].keys()) == list(
                hierarchy[attribute].keys()
            )  # True if full attribute in rules
            ...
            if if_list_selected:
                ...
                if not full_att:
                    var_selected_list = list(
                        result_dict[a].keys()
                    )  # list of variants from 'NOT'
                    att_selected_list = list(
                        hierarchy[a].keys()
                    )  # full list of variants from hierarchy attribute

                    # Invert Variants set in NOT rule:
                    var_selected_list = [
                        i for i in att_selected_list if i not in var_selected_list
                    ]
                    ...
                    deconstructed_dna[int(attribute_index)] = logic_rarity(
...
def get_rule_break_type(
    hierarchy, deconstructed_dna, if_dict, result_dict, result_dict_type
):
    """
    ...
    for a in if_dict:  # Attribute in 'if_dict'
        for b in if_dict[a]:  # Variant in if_dict[Attribute]
            ...
            dna_order_num = str(
                deconstructed_dna[if_dict[a][b][4]]
            )  # Order Number of 'b's attribute in deconstructed_dna

            if (
                var_order_num == dna_order_num
            ):  # If DNA selected Variants found inside IF list variants:
                if_bool = True
                break
        else:
            continue
    ...
    for a in result_dict:  # Attribute in 'result_dict'
        for b in result_dict[a]:  # Variant in if_dict[Attribute]
            ...
            dna_order_num = str(
                deconstructed_dna[result_dict[a][b][4]]
            )  # Order Number of 'b's attribute in deconstructed_dna
            if (
                var_order_num == dna_order_num
            ):  # If DNA selected Variants found inside THEN list variants:
...
def create_dicts(hierarchy, rule_list_items, result_dict_type):
    ...
    for a in rule_list_items:
        for b in hierarchy:
            if (
                a == b
            ):  # If 'a' is an Attribute, add all 'a' Variants to items_returned dict.
            ...
            if a in list(
                hierarchy[b].keys()
            ):  # If 'a' is a Variant, add all info about that variant to items_returned
    ...
    while did_reconstruct:
        ...
        for rule in logic_file:
            ...
            if "THEN" in logic_file[rule]:
                ...
                result_dict = create_dicts(
                    hierarchy, logic_file[rule][result_dict_type], result_dict_type
                )
